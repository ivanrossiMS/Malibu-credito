  ...
  <div class="sticky top-0 z-40 bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100 -mx-6 px-4 py-3 mb-8">
    ...
    <div class="hidden lg:flex items-center gap-3 flex-wrap">
      ...
      <div class="flex items-center gap-1 bg-slate-100 p-1 rounded-xl">
        ...
        <button onclick="dash_setPeriod('mes', this)" data-period="mes" class="dash-period-btn px-3 py-1.5 rounded-lg text-[11px] font-black uppercase tracking-wider transition-all bg-white text-primary shadow-sm">Mês</button>
      ...
      <div class="relative flex-1 min-w-[180px]">
        ...
        <input id="dash-search" type="text" placeholder="Cliente, CPF, contrato..." oninput="dash_onSearch(this.value)" class="w-full pl-8 pr-4 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium text-slate-700 placeholder:text-slate-400 outline-none focus:border-primary transition-colors">
      </div>
    ...
    <div class="flex lg:hidden items-center gap-3">
      <div class="relative flex-1">
        <i data-lucide="search" class="w-3.5 h-3.5 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2 pointer-events-none"></i>
        <input id="dash-search-mobile" type="text" placeholder="Buscar..." oninput="dash_onSearch(this.value)" class="w-full pl-8 pr-4 py-2 bg-slate-100 rounded-xl text-xs text-slate-700 outline-none">
      </div>
    ...
    <div class="premium-card p-6 border-indigo-100/50 bg-gradient-to-br from-white to-indigo-50/30">
      <div class="flex items-center justify-between mb-5">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="text-sm font-black text-slate-900">Pendências</h3>
            <p class="text-[10px] text-indigo-500 font-bold uppercase tracking-wider">Do sistema</p>
          </div>
        </div>
        <span id="qa-pending-badge" class="hidden bg-rose-500 text-white text-[9px] font-black px-2 py-0.5 rounded-full">!</span>
      </div>
      <div id="qa-pending-list" class="space-y-2">
        <div class="text-center py-4 text-slate-400 text-xs">
          <i data-lucide="shield-check" class="w-5 h-5 mx-auto mb-1 text-emerald-300"></i>
          Tudo em ordem
        </div>
      </div>
    </div>
      ...
      <div class="premium-card p-6">
        <div class="flex items-center justify-between mb-4">
          <div class="flex items-center gap-3">
            ...
            <h3 class="text-sm font-black text-slate-900">ASAAS</h3>
          </div>
          <div id="asaas-health-indicator" class="flex items-center gap-2">
            <div class="w-2 h-2 rounded-full bg-slate-300 animate-pulse"></div>
            <span class="text-[10px] font-black text-slate-400 uppercase tracking-wider">Verificando...</span>
          </div>
        </div>
        <div id="asaas-health-detail" class="text-[10px] text-slate-400 space-y-1">
          <p>Verificando integração...</p>
        </div>
        <button onclick="window.location.href='?page=companies'" id="asaas-config-btn" class="hidden mt-3 w-full text-[10px] font-black text-primary border border-primary/20 bg-primary/5 py-2 rounded-xl hover:bg-primary/10 transition-colors">
